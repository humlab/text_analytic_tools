...
class SouXmlHandler(xml.sax.ContentHandler):
    ...
    def get_lemma(self, attrs):
        ...
        # CHANGE 201702013: PREFIX/SUFFIX does not exist in Stanford tagger XML
        # if lemma == None:
    ...
    # CHANGE 201702013: Not applicable to Stanford tagger XML
    ...
    def startElement(self, name, attrs):
        ...
        if name == "corpus" or name == "text":
            ...
            # CHANGED 20170212: Adapted to Stanfrod tagger XML
            title = os.path.splitext(os.path.basename(self.options["source_file"]))[0]
            m = re.search('_(?P<year>\d{4})_', self.options["source_file"])
...
if __name__ == "__main__":
    ...
    # Note: The following data must be configured to each specific case
    # Place select different location for source and destination
    options = {
        "source": "/tmp/output/temp/",                       # test
        "destination": "/tmp/output/temp/",            # test
        "years_of_interest": range(1950, 2016),     # Ignore "text" tags having an ID not found in list

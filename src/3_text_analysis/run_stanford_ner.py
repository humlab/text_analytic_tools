...
def merge_entities(entities):
    ...
    for doc_id, i_n, w_n, t_n in entities[1:]:
        ...
        if doc_id == doc_id_p and i_n == i_p + 1 and t_n == t_p:
...
def compute_stanford_ner(source_file, service_url=STANFORD_CORE_NLP_URL, excludes=None):
    ...
    for filename, text, metadata in stream:
        document_id = document_index.loc[document_index.filename == filename].document_id[0]
        ner_data = recognize_named_entities(tagger, document_id, text, excludes)
        i += 1
        if i % 10:
